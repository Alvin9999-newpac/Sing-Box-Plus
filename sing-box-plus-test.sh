...
if command -v ufw >/dev/null 2>&1 && ufw status | grep -q -E "active|活跃"; then
  # 若 UFW 关闭了 IPv6，会导致所有 IPv6 节点无法连接（端口规则只作用于 IPv4）
  if [[ -f /etc/default/ufw ]] && grep -q "^IPV6=no" /etc/default/ufw; then
    sed -i 's/^IPV6=.*/IPV6=yes/' /etc/default/ufw || true
    systemctl restart ufw >/dev/null 2>&1 || service ufw restart >/dev/null 2>&1 || true
  fi
...
if [[ "$proto" == tcp ]]; then
  ...
  command -v ip6tables >/dev/null 2>&1 && ( ip6tables -C INPUT -p tcp --dport "$p" -j ACCEPT 2>/dev/null || ip6tables -I INPUT -p tcp --dport "$p" -j ACCEPT ) || true
fi
if [[ "$proto" == udp ]]; then
  iptables  -C INPUT -p udp --dport "$p" -j ACCEPT 2>/dev/null || iptables  -I INPUT -p udp --dport "$p" -j ACCEPT
  command -v ip6tables >/dev/null 2>&1 && ( ip6tables -C INPUT -p udp --dport "$p" -j ACCEPT 2>/dev/null || ip6tables -I INPUT -p udp --dport "$p" -j ACCEPT ) || true
fi
    done
